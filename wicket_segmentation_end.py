...
def apply_crop_to_video_for_last_wicket(video_path, roi, extraction_interval, start_second, score_position, separator):
    """
    ...
    Args:
        ...
        start_second (int): Starting second for cropping.
        score_position (str): Position of the score relative to the wicket.
        separator (str): Separator used in the score.
    ...
    start_frame = int(start_second * fps)  # Calculate start frame based on start_second

    i = start_frame  # Initialize counter
    while i < duration * fps:
        ...
        if ret:
            ...
            if is_last_wicket(ocr_result):
                ...
                return seconds
            i += frame_interval  # Increment the counter by frame interval
        else:
    ...
    while i < duration * fps:
        ...
        if ret:
            ...
            if is_score(ocr_result, score_position, separator):
                ...
                #     # Adjust frame interval after 9th wicket to get more frames
                #     frame_interval = max(int(fps * extraction_interval / 10), 1)
                #     if is_last_wicket(ocr_result):
                #         print('10 wicket')
...
def process_video(video_path, extraction_interval, output_folder):
    ...
    if (score_position == 'R' or score_position == 'L' or score_position == 'r' or score_position == 'l'):
        ...
        last_wicket = score[-1][1]
        if last_wicket == 9:
            extraction_interval_for_tenth_wicket = 0.5
            tenth_wicket_second = apply_crop_to_video_for_last_wicket(video_path, roi,
                                                                      extraction_interval_for_tenth_wicket,
                                                                      score[-1][0], score_position, separator)
            if tenth_wicket_second != -1:
                score.append((tenth_wicket_second, 10))
...
if __name__ == "__main__":
    input_video = "australia.mp4"
    output_folder = "wickets"
    interval = 10  # Interval for frame extraction
