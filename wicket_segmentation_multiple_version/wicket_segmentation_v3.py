...
def find_wicket_number(text,score_position):
    """
    ...
    if hyphen_index != -1:
        ...
        if score_position == "R" or score_position == "r":
            if text_length> (hyphen_index+1) and text[hyphen_index+1] == '1' and text[hyphen_index+2] == '0':
                try:
                    wicket = int(10)
            ...
            if (hyphen_index>1) and text[hyphen_index-2] == '1' and text[hyphen_index-1] == '0':
                try:
                    wicket = int(10)
                except ValueError:  # If after hyphen it is not an integer digit then assign -1 to wicket
                    wicket = -1
...
def is_score(ocr_result,score_position):
    """
    ...
    """
    if score_position=="R" or score_position =="r":
        pattern = r".*(\d{1,3})-(\d).*"  # Regular expression pattern to match score
    else:
        pattern = r".*(\d)-(\d{1,3}).*"
...
def apply_crop_to_video(video_path, output_folder, roi, extraction_interval,score_position):
    """
    ...
    for i in range(0, duration * fps, frame_interval):
        ...
        if ret:
            ...
            if is_score(ocr_result,score_position):
                # Extract wicket information from OCR result
                wicket_in_this_frame = int(find_wicket_number(ocr_result,score_position))
...
def process_video(video_path, extraction_interval, crop_output_folder, output_folder):
    ...
    Args:
        video_path (str): Path to the input video file.
        extraction_interval (int): Interval for frame extraction.
        crop_output_folder (str): Path to save the cropped frames.
        output_folder (str): Path to save the clipped videos.
    """
    ...
    # Todo  Take input from user left or right
    # user give:
    ...
    if(score_position== 'R' or score_position == 'L' or score_position== 'r' or score_position == 'l'):
        ...
        score = apply_crop_to_video(video_path, crop_output_folder, roi, extraction_interval, score_position)
...
if __name__ == "__main__":
    ...
    interval = 2  # Interval for frame extraction
